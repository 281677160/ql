...
if [ "$(grep -c wskey.py /ql/config/crontab.list)" = 0 ]; then
    echo
    echo
    TIME g "开始添加 [每8小时运行WSKEY转换]"
    echo
    echo
    # 获取token
    token=$(cat /ql/config/auth.json | jq --raw-output .token)
    curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" -H 'Content-Type: application/json;charset=UTF-8' -H 'Accept-Language: zh-CN,zh;q=0.9' --data-binary '{"name":"每8小时运行WSKEY转换","command":"task wskey.py","schedule":"* */8 * * *"}' --compressed 'http://127.0.0.1:5700/api/crons?t=1633428022377'
fi
