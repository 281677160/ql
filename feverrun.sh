...
if [ "$(grep -c extra /ql/config/crontab.list)" = 0 ]; then
    ...
    curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" -H 'Content-Type: application/json;charset=UTF-8' -H 'Accept-Language: zh-CN,zh;q=0.9' --data-binary '{"name":"每天检测任务更新","command":"ql extra","schedule":"15 23 * * *"}' --compressed 'http://127.0.0.1:5700/api/crons?t=1624782068473'
fi
