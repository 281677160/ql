#!/usr/bin/env bash
  ...
  rm -rf nolanjdc
  echo -e "\033[32m 下载源码，请耐心等候... \033[1m"
  ...
  cd /root
  echo -e "\033[32m 安装镜像，请耐心等候... \033[1m"
  ...
  if [[ `docker ps -a | grep -c "nvjdc"` -ge '1' ]]; then
    echo -e "\033[32m nvjdc镜像启动成功 \033[1m"
  else
    echo -e "\033[31m nvjdc镜像启动失败 \033[1m"
    exit 1
  fi
  timeout -k 1s 4s docker logs -f nolanjdc
  echo -e "\033[32m nvjdc升级完成 \033[1m"
  exit 0
