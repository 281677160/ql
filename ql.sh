...
if [[ `docker ps -a | grep -c "qinglong"` -ge '1' ]]; then
	if [[ -n "$(ls -A "${QL_PATH}/qlbeifen1" 2>/dev/null)" ]]; then
		...
		curl -fsSL https://ghproxy.com/https://raw.githubusercontent.com/281677160/ql/main/feverrun/authbk.json > ${QL_PATH}/ql/authbk.json
		sleep 2
	...
	if [[ `docker exec -it qinglong bash -c "cat /ql/config/auth.json" | grep -c "\"token\""` -ge '1' ]]; then
		...
		echo
		TIME y "请使用您的 IP:${QL_PORT} 在浏览器打开页面,登录青龙面板"
		echo
